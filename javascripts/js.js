...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener ('drop', (event) => {
    ...
    const imgUrl = event.dataTransfer.getData('text/uri-list');
    const newLeftVw = pxToVw(event.clientX - offsetX);
    const newTopVh = (event.clientY - offsetY) / window.innerHeight * 100 +  394;
    currentDraggedElement.style.left = `${newLeftVw}vw`;
    currentDraggedElement.style.top = `${newTopVh}vh`;
    console.log('изображение помещено', imgUrl);
    console.log(`Изображение перемещено в: ${event.clientX}, ${event.clientY}`);
  });

  window.addEventListener('resize', function() {
    // Можно добавить логику для корректировки позиций при ресайзе
